...
void sapXepMangNhap(int arr[],int n){
 ...
 for(i=0;i<n;i++){
  printf("\n arr[%d]  sap xep lai theo thu tu be den lon la: %d\n",i,arr[i]);
 }
}
...
int timMin(int arr[], int n){ // co the viet arr[] <-> *arr
	int m=arr[0];
	for (int i=0;i<n;i++){
		if (m>arr[i]){
			m=arr[i];
...
int timMaxNT (int arr[], int n){
	int max=0;
	for (int i=0;i<n;i++){
	max= timMax(arr,n);
	}
	max=max+1;
	while (checkSNT(max)!=true){
		max++;
	}
		return max;
	}
	
int UclnMang (int arr[],int n){
	int x= arr[0];
	for (int i=1;i<n;i++){
		x=timUcln(x,arr[i]);
	}
...
int BcnnMang (int arr[], int n){
	int y=timBcnn(arr[0],arr[1]);
	for (int i=2;i<n;i++){
		y=timBcnn(y,arr[i]);
	}
	return y;
